...
class MonteCarloProvider extends ChangeNotifier {
  ...
  Future<void> runMultiQSimulation() async {
    ...
    for (final q in qValues) {
      List<double> repetitionAverageProfits = [];
      ...
      for (int i = 0; i < _multiQRepetitions; i++) {
        ...
        if (isValid && randomNumbers != null) {
          ...
          repetitionAverageProfits.add(avgProfit);
        } else {
          _simulationError = "La simulación para Q=$q (repetición ${i + 1}) falló. No se pudo generar una secuencia válida.";
      ...
      if (allRepetitionsSuccessful) {
        final double grandAverageProfit = repetitionAverageProfits.reduce((a, b) => a + b) / repetitionAverageProfits.length;
        _multiQResults.add(MultiQResult(
          quantity: q!,
          averageProfit: grandAverageProfit,
        ));
      } else {
        break;
      }
